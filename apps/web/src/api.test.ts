...
describe("generateHistory", () => {
  ...
  it("sends a war-series request without topic input", async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ({
  ...
  it("uses a war-series instruction when the dedicated endpoint is missing", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({ ok: false, status: 404 })
      .mockResolvedValueOnce({
    ...
    await generateHistory({ mode: "war" });

    expect(JSON.parse(String(fetchMock.mock.calls[1]?.[1]?.body))).toMatchObject({
      message: expect.stringContaining("具体历史战争或战役")
    });
  });
});
