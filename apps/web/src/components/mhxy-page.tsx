            ...
            <div className="mhxy-assets__board">
                    ...
                    {holdingAssetFlips.map((item) => (
                      <article className="mhxy-asset-row" key={item.id}>
                        ...
                        <span className="mhxy-row-actions">
                          <button type="button" onClick={() => { setEditingAssetFlipId(item.id); setAssetFlip({ ...item, purchaseCurrency: "rmb", gameCoinCost: undefined, buyAt: toLocalDateTimeInput(item.buyAt), sellAt: item.sellAt ? toLocalDateTimeInput(item.sellAt) : "", sellPriceRmb: item.sellPriceRmb }); }}>编辑</button>
              ...
              <form className="mhxy-asset-editor" data-form="asset-flip" onSubmit={submitAssetFlip}>
                ...
                <div className="mhxy-asset-preview">
                  ...
                  <small>未填写卖出信息时，这条记录会按人民币买入成本计入库存价值。</small>
                </div>
